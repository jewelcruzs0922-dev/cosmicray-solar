...
export default function ClientHome() {
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    form.querySelectorAll(".form-group input").forEach((el) => {
    ...
    if (!nameEl.value.trim()) {
      ...
      nameEl.setAttribute("aria-describedby", "name-error");
      valid = false;
    }
    if (!emailEl.value.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailEl.value)) {
      ...
      emailEl.setAttribute("aria-describedby", "email-error");
      valid = false;
    }
    if (!phoneEl.value.trim()) {
      ...
      phoneEl.setAttribute("aria-describedby", "phone-error");
      valid = false;
    }
    if (!valid) return;
